...
export async function loadCalculationForDocument(tenantId: string, calculationId: string) {
  const calc = await prisma.calculation.findFirst({
    ...
    include: {
      ...
      fixedPricePositions: { orderBy: { sortOrder: "asc" } },
      vatSettings: true,
      customer: true,
  ...
  const docCalc: DocumentCalcInput = {
    ...
    calculatedNetSalesPrice: calc.engineNetSalesPrice ?? calc.netSalesPrice,
    fixedPricePositions: calc.fixedPricePositions.map((p) => ({
