...
function MonteurTagesplanViewContent() {
  ...
  async function updateStatus(appointmentId: string, status: string) {
    ...
    if (data.success) {
      ...
      toast.error(data.error ?? "Status konnte nicht gespeichert werden");
    }
  }
  ...
  async function respondRequest(requestId: string, action: "accept" | "decline") {
    ...
    const data = await res.json();
    if (data.success) {
      toast.success(action === "accept" ? "Anfrage angenommen" : "Anfrage abgelehnt");
      loadSchedule();
      loadRequests();
    } else {
  ...
  async function startPause(orderId: string) {
    const res = await fetch(`/api/monteur/orders/${orderId}/time`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await res.json();
    if (data.success) {
      toast.success("Pause erfasst");
    } else {
      toast.error(data.error ?? "Pause konnte nicht erfasst werden");
    }
  }
  ...
  async function uploadPhoto(orderId: string, file: File, category = "VORHER") {
    try {
      ...
      const res = await fetch(`/api/monteur/orders/${orderId}/files`, { method: "POST", body: formData });
      const data = await res.json();
      if (!data.success) {
...
export function MonteurTagesplanView() {
  return (
    <Suspense fallback={<p className="text-slate-500">Laden...</p>}>
      <MonteurTagesplanViewContent />
    </Suspense>
  );
}
