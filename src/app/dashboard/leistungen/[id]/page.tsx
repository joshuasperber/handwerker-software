...
export default function LeistungDetailPage() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Initialer, asynchroner Leistungsabruf.
    load();
  }, [load]);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <Link
          ...
          className="flex items-center gap-1 text-sm text-[#0d5c63] hover:underline"
        >
          <ChevronLeft className="h-4 w-4" /> Zurück zu Leistungen
        </Link>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-1">
        ...
        >
          {service.isActive ? "Aktiv" : "Deaktiviert"}
        </span>
      </div>
      ...
      <Card title="Material-Stückliste (nur Inventar-Artikel)">
        <p className="text-sm text-slate-500 mb-4">
          Materialpositionen müssen aus dem Inventar stammen. Werkzeuge werden separat als
          nicht-reservierbare Positionen geführt.
        </p>
        ...
        <form onSubmit={addMaterial} className="border-t border-slate-100 pt-4 space-y-3">
          ...
          {articles.length === 0 && (
            <p className="text-xs text-amber-700">
              ...
              <Link href="/dashboard/inventar" className="underline">
                Inventar
              </Link>{" "}
              anlegen.
            </p>
          ...
          <Button type="submit" size="sm">
            <Plus className="h-4 w-4 mr-1" /> Material hinzufügen
          </Button>
        </form>
      </Card>
      ...
      <Card title="Werkzeuge (nicht reservierbar)" className="mt-6">
          ...
          {toolLines.map((t) => (
            <div key={t.id} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {t.name} · {t.defaultQuantity} {t.unit}
                </span>
              </div>
              <button
                type="button"
        ...
        <form onSubmit={addTool} className="flex flex-wrap gap-2 items-end">
          ...
          <NumberInput
            label="Anzahl"
            min={1}
            value={toolForm.defaultQuantity}
            onValueChange={(v) => setToolForm({ ...toolForm, defaultQuantity: v ?? 1 })}
          />
